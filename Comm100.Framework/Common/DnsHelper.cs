...
namespace Comm100.Framework.Common
{
    public class DnsHelper
    {
        ...
        public static List<string> GetTextRecords(string domain)
        {
            ...
            try
            {
                var result = DnsQuery(ref domain, DnsRecordTypes.DNS_TYPE_TEXT, DnsQueryOptions.DNS_QUERY_BYPASS_CACHE,
                IntPtr.Zero, ref recordsArray, IntPtr.Zero);
                if (result != 0) // timeout and other unexpected results.
                {
                    throw new Exception("timeout and other unexpected results.");
                }

                DnsRecordTxt record;
                var recordList = new List<string>();
                for (var pointer = recordsArray; !pointer.Equals(IntPtr.Zero); pointer = record.pNext)
                {
                    record = (DnsRecordTxt)Marshal.PtrToStructure(pointer, typeof(DnsRecordTxt));
                    if (record.wType == (int)DnsRecordTypes.DNS_TYPE_TEXT)
                    {
                        var lines = new List<String>();
                        var stringArrayPointer = pointer
                          + Marshal.OffsetOf(typeof(DnsRecordTxt), "pStringArray").ToInt32();

                        for (var i = 0; i < record.dwStringCount; ++i)
                        {
                        ...
                        recordList.Add(String.Join("", lines));
                    }
                }
                return recordList;
            }

...
namespace Comm100.Framework.Common
{
    public class CacheHelper
    {
        ...
        public static void CacheInsertAddMinutes<T>(string key, T value, int minute)
        {
        ...
        public static void CacheInsertFromMinutes<T>(string key, T value, int minute)
        {
            if (value == null) return;
            _memoryCache.Set(key, value, new MemoryCacheEntryOptions()
        ...
        ///<param name="key">Cache键值</param>
        ///<param name="value">给Cache[key]赋的值</param>
        public static void CacheInsert<T>(string key, T value)
        {
